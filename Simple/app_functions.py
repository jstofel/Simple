...
def getPageInfo(page_id, conn):
    if int(page_id) > 0:
        ...
        psql = "select s.page_id , s.page_name, s.page_title, s.page_template, s.page_indent, " ;
        psql += " case when length(btrim(s.content_md)) > 1 then length(btrim(s.content_md)) else 0 end as has_content, ";
        ...
        psql += "case when p2.page_template != 'index' then p2.page_template else '\' end  as prev_page_template, ";
        psql += "(select max(page_id) as max_page from page) as max_page ";
        psql += " from " ;
        ...
        psql = "select page_id, page_name, page_title, page_template, page_indent, 0 as has_content , " ;
        psql += "0 as next_page_id, 'content' as next_page_template, ";
        psql += "0 as prev_page_id, 'content' as prev_page_template , ";
        psql += "(select max(page_id) as max_page from page) as max_page ";
        psql += "from public.page ";
        psql += "order by page_order";

    result = conn.execute(psql);
    fetchall = result.fetchall()
    import pandas as pd
    pageInfo = pd.DataFrame(fetchall, columns=['page_id', 'page_name', 'page_title', 'page_template', 'page_indent', 'has_content', 'next_page_id', 'next_page_template',  'prev_page_id', 'prev_page_template', 'max_page'])
    return pageInfo
