...
def getPageInfo(page_id, conn):
    if int(page_id) > 0:
        ...
        psql = "select page_id, page_name, page_title, page_template, page_level, " ;
    ...
    import pandas as pd
    pageInfo = pd.DataFrame(fetchall, columns=['page_id', 'page_name', 'page_title', 'page_template', 'page_level', 'next_page_id', 'next_page_template',  'prev_page_id', 'prev_page_template'])
    return pageInfo
